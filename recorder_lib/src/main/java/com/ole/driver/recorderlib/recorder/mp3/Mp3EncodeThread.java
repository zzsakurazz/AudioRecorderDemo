package com.ole.driver.recorderlib.recorder.mp3;
...
public class Mp3EncodeThread extends Thread {
    ...
    public Mp3EncodeThread(File file, int bufferSize) {
        ...
        Logger.w(  "in_sampleRate:%s，getChannelCount:%s ，out_sampleRate：%s 位宽： %s,",
    ...
    private void lameData(ChangeBuffer changeBuffer) {
        ...
        if (readSize > 0) {
            ...
            if (encodedSize < 0) {
                Logger.e(  "Lame encoded size: " + encodedSize);
            }
            try {
                os.write(mp3Buffer, 0, encodedSize);
            } catch (IOException e) {
                Logger.e(e,   "Unable to write to file");
            }
        }
    }
